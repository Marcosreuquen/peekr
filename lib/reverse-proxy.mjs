    ...
    req.on('end', async () => {
      ...
      if (rule) {
        ...
        if (rule.action === 'mock') {
          ...
          if (onRequest) onRequest({ id, direction: 'IN', method: req.method, url: req.url, host, timestamp, reqHeaders: req.headers, reqBody: body, statusCode: mockStatus, resHeaders: mockHeaders, resBody: mockResponseBody, durationMs: 0, responseSize: Buffer.byteLength(mockResponseBody), ruleAction: 'mocked' });
          return;
        }
      }
      ...
      applyTransform('request', 'IN', reqCtx);

      // ── Request-phase breakpoint ──
      const bpReqRule = findMatch(host, req.method, req.url, 'IN');
      if (bpReqRule && bpReqRule.action === 'breakpoint') {
        const bpPhase = bpReqRule.phase || 'both';
        if (bpPhase === 'request' || bpPhase === 'both') {
          const resolution = await pauseAtBreakpoint({ ...reqCtx, direction: 'IN', phase: 'request', timeoutMs: bpReqRule.timeoutMs ?? 30000 });
      ...
      const fwdReq = http.request(
        ...
        async fwdRes => {
          ...
          fwdRes.on('end', async () => {
            ...
            applyTransform('response', 'IN', resCtx);

            // ── Response-phase breakpoint ──
            const bpResRule = findMatch(host, req.method, req.url, 'IN');
            if (bpResRule && bpResRule.action === 'breakpoint') {
              const bpPhase = bpResRule.phase || 'both';
              if (bpPhase === 'response' || bpPhase === 'both') {
                const resolution = await pauseAtBreakpoint({ ...resCtx, direction: 'IN', phase: 'response', timeoutMs: bpResRule.timeoutMs ?? 30000 });
                if (resolution.action === 'block') {
                  const blockBody = JSON.stringify({ error: 'blocked at breakpoint' });
                  res.writeHead(403, { 'Content-Type': 'application/json' });
                  res.end(blockBody);
