...
export function createProxyServer(opts = {}) {
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on('end', async () => { try {
      ...
      // ── Dynamic rules (block / modify / breakpoint) ──
      ...
      if (rule) {
        ...
        if (rule.action === 'modify' && rule.modifyConfig?.noForward) {
          ...
          if (onRequest) onRequest({ id, method: req.method, url: upstreamPath, host: destHost, timestamp, reqHeaders: cleanReqHeaders, reqBody: body, statusCode: mockStatus, resHeaders: mockResHeaders, resBody: mockResponseBody, durationMs: 0, responseSize: Buffer.byteLength(mockResponseBody), direction: 'OUT', ruleAction: 'mocked' });
          return;
        }
      }
      ...
      // rule from above; block/mock already returned, so breakpoint falls through
      if (rule && rule.action === 'breakpoint') {
          const resolution = await pauseAtBreakpoint({ ...reqCtx, direction: 'OUT', phase: 'request', timeoutMs: rule.timeoutMs ?? 30000 });
      ...
      if (noForward && !filtered) {
        ...
        if (!uiMode) {
          logResponse({ statusCode: 200, body: JSON.stringify(mockResponse) });
        }
      ...
      const upstreamHost = (target && !filtered) ? target : destHost;
      const upstreamPort = (target && !filtered) ? 443 : destPort;
      const useHttps     = (target && !filtered) ? true : (destProto === 'https');
      ...
      const forwardReq = transport.request(
        ...
        async forwardRes => { try {
          const fwdChunks = [];
          forwardRes.on('data', chunk => fwdChunks.push(chunk));
          forwardRes.on('end', async () => { try {
            ...
            if (rule && rule.action === 'breakpoint') {
                const resolution = await pauseAtBreakpoint({ ...resCtx, direction: 'OUT', phase: 'response', timeoutMs: rule.timeoutMs ?? 30000 });
                if (resolution.action === 'block') {
                  const blockBody = JSON.stringify({ error: 'blocked at breakpoint' });
                  res.writeHead(403, { 'Content-Type': 'application/json' });
                  res.end(blockBody);
            ...
            console.error(c('red', `\n[#${id}] UNHANDLED ERROR in response handler: ${err.message}`));
          ...
          console.error(c('red', `\n[#${id}] UNHANDLED ERROR in forwardRes callback: ${err.message}`));
          if (!res.headersSent) { res.writeHead(500, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ error: 'internal proxy error', detail: err.message })); }
        }
      }
      );
      ...
      console.error(c('red', `\n[#${id}] UNHANDLED ERROR in request handler: ${err.message}`));
      if (!res.headersSent) { res.writeHead(500, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ error: 'internal proxy error', detail: err.message })); }
    }
    });
  });
