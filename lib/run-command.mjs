...
export async function runCommand(argv) {
  ...
  if (logFile)   console.log(`Log file      ${c('cyan', logFile)}`);
  if (ignore.length) console.log(`Ignoring      ${c('dim', ignore.join(', '))}`);;
  console.log(c('dim', '─'.repeat(40)));
  console.log('');
  console.log(c('bold', 'How it works:'));
  console.log(`  All outgoing HTTP/HTTPS calls from the child process are automatically`);
  console.log(`  intercepted and logged. No changes needed in your app or .env files.`);
  console.log(`  Works with Axios, fetch, undici, got, and any Node.js HTTP client.`);
  console.log('');
  console.log(c('dim', '─'.repeat(40)) + '\n');
