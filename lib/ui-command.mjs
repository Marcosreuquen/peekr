...
export async function uiCommand(argv) {
  ...
  const appPort     = resolvePort(opts, config, 'app-port', ['app', 'appPort'], 3000);
  const port        = resolvePort(opts, config, 'port', ['proxy', 'proxyPort', 'port'], 49999);
  const reversePort = resolvePort(opts, config, 'reverse-port', ['reverseProxy', 'reverseProxyPort', 'reversePort'], 49998);
  const uiPort      = resolvePort(opts, config, 'ui-port', ['ui', 'uiPort'], 49997);
  ...
  if (process.stdin.isTTY) {
    // Keep the terminal in canonical mode. Raw mode turns Ctrl+C into a regular
    // byte instead of SIGINT; when a child also inherits stdin, that child can
    // consume the byte and leave peekr running indefinitely.
