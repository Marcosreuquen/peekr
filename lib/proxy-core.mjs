      ...
      if (bpReqRule && bpReqRule.action === 'breakpoint') {
        ...
        if (bpPhase === 'request' || bpPhase === 'both') {
          ...
          if (resolution.action === 'mock') {
            ...
            res.end(mockBody2);
            if (onRequest) onRequest({ id, method: req.method, url: upstreamPath, host: destHost, timestamp, reqHeaders: reqCtx.headers, reqBody: reqCtx.body, statusCode: mockStatus, resHeaders: mockHeaders2, resBody: mockBody2, durationMs: 0, responseSize: Buffer.byteLength(mockBody2), direction: 'OUT', ruleAction: 'mocked' });
            return;
          }
